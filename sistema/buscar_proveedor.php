...
<body>
	...
	<section id="container">
		...
		<a href="registro_usuario.php" class="btn_new">Crear Proveedores</a>
		...
		<table>
			...
			$sql_registe = mysqli_query($conection,"SELECT COUNT(*) AS total_registro FROM proveedor 										WHERE ( codproveedor LIKE '%$busqueda%' OR 														proveedor    LIKE '%$busqueda%' OR 												        contacto	 LIKE '%$busqueda%' OR 												        telefono     LIKE '%$busqueda%' 	 )				 									AND status = 1");
			...
			$query = mysqli_query($conection,"SELECT * FROM proveedor WHERE 																				( codproveedor LIKE '%$busqueda%' OR											 		  proveedor	   LIKE '%$busqueda%' OR 										               contacto     LIKE '%$busqueda%' OR 										    		    telefono     LIKE '%$busqueda%' )													                   AND  	status = 1 											 ORDER BY codproveedor ASC LIMIT $desde,$por_pagina");
			...
			<!-- el codigo de php me crea un superusuario que no puede ser borrado de la lista de usuarios -->
					...
					|
					<a class="link_delete" href="eliminar_confirmar_proveedor.php?id=<?php echo $data["codproveedor"]; ?>" >borrar</a>
			</td>
			</tr>
			<?php
